namespace Spiegel_VN {
  export async function Chp04_CS_EntryForest(): ƒS.SceneReturn {
    ...
    await ƒS.Speech.tell(characters.maincharacter, "Schnaufend schleppst du dich von Stamm zu Stamm, über Wurzeln und Gebüsch. Du hältst kurz an, um zu verschnaufen, und stehst plötzlich inmitten einer Lichtung. Der Mond scheint hell von oben herab.");
    await ƒS.Location.show(locations.Chp05_Wishtree);
    await ƒS.update(transitions.fade.duration, transitions.fade.alpha, transitions.fade.edge);
    await ƒS.Speech.tell(characters.maincharacter, "Das Licht ist so hell, dass du erst nach einigen Sekunden den mächtigen Baum siehst, der vor dir steht.");
    ...
    await ƒS.Speech.tell(characters.maincharacter, "Wie kann das sein? Also ist es wahr: es gibt einen echten Baum der Wünsche? Deine Stimme ist heiser, als du sprechen willst.");
