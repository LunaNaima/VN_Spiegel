    ...
    //   }
      ...
      switch (chp01FlowerMerchantDialogueElement) {
        case chp01FlowerMerchantDialogueElementAnswers.iSayAskAboutTrip:
          ...
          await ƒS.Speech.tell(characters.flowerMerchant.name,'"Zum Glück habe ich so ein feines Näschen.Damit konnte ich das Gesindel meilenweit voraus riechen! Dann haben wir Händler einen anderen Weg genommen.Schau nicht so zweifelnd, jedes Wort, das ich gesprochen habe, ist wahr, jaja!."');
          ƒS.Speech.clear();
        ...
        case chp01FlowerMerchantDialogueElementAnswers.iSayAskAboutFlowers:
          ...
          await ƒS.Speech.tell(characters.flowerMerchant.name, '"Ach die Blumen! Dieses Jahr werden sie mir förmlich aus den Händen gerissen. Es scheint so, als würden sich immer mehr Leute meine wunderschönen Kreationen zuhause aufstellen."')
           await ƒS.Speech.tell(characters.flowerMerchant.name, '"Seltsamerweise kaufen die grauesten Mäuse die buntesten Blumen… Nicht, dass ich mich beschwere! Nun werden sich eben diese, naja, kleinen Dörfchen wie eures mit weniger zufrieden geben müssen."');
          ƒS.Speech.clear();
          return "01_E_FlowerMerchant";
          break;
