namespace Spiegel_VN {
  export async function Chp01_CS_PerchaseMirror(): ƒS.SceneReturn {
    ...
    ƒS.update();
    await ƒS.Speech.tell(characters.mirrorMerchant, '"Darf ich Ihnen den magischen Spiegel vorführen? Lassen Sie sich verzaubern! Es funktioniert so: Sie halten sich den Spiegel vor das Gesicht und sagen “Spieglein, Spieglein, weise mir mein Träumlein” hinein, und dann dürfen Sie sich etwas wünschen."');
    ...
    ƒS.update();
    await ƒS.Speech.tell(characters.mirrorMerchant, '"Schauen Sie, wie brillant die Farben sind! Und das Schloss, eine wahre Pracht. Was meinen Sie, für zwei Goldstücke ist das doch eine unglaubliche Gelegenheit?"');
    await ƒS.Character.show(characters.Mama, characters.Mama.pose.dress_34_neutral, ƒS.positionPercent(70, 100));
    ƒS.update();
    await ƒS.Speech.tell(characters.Mama, '"Für mich ist so eine Spielerei nichts! Aber vielleicht für Mutti …?"');
    await ƒS.Location.show(locations.Chp01_03_IntroMirror);
    ƒS.update();
    await ƒS.Speech.tell(characters.mirrorMerchant, '"Die Spiegel eignen sich wunderbar als Geschenk für Ihre Frau! Steht ihr denn eher Gold oder Silber? Ich habe auch Spiegel aus anderen Edelmetallen im Angebot."');
    await ƒS.Speech.tell(characters.Mama, '"Gold ist eher ihre Farbe. Sie ist Künstlerin, wissen Sie; mit diesem Spiegel könnte sie vielleicht ihre Gedanken besser visualisieren, manchmal fehlt ihr das."');
    await ƒS.Speech.tell(characters.mirrorMerchant, '"Das denke ich doch auch! Als Inspiration dient der Spiegel geradezu vorzüglich! Und für nur zwei Goldstücke machen Sie Ihre Frau glücklich."');
    await ƒS.Speech.tell(characters.Mama, '"Na gut, wir probieren es mal aus! Dann nehmen wir eben diesen hier."');
